...
def get_most_common_color_class(regions: List[np.ndarray], threshold: float = 0.05) -> int:
    """
    ...
    for region in regions:
        ...
        # 标记类别：1=红主导，0=其他
        if red_ratio > threshold and red_ratio > green_ratio*0.02:
            count_1 += 1
        else:
            count_0 += 1
...
class SCAlgorithmBase:
    ...
    def _load_model(self):
        """加载YOLO11分类模型（子类无需重写，仅模型路径不同），加载后立即warmup"""
        import time
        try:
            ...
            # warmup：用128x128灰度图进行一次前向，避免首次推理耗时3s+
            t0 = time.time()
            dummy = np.zeros((128, 128, 3), dtype=np.uint8)
            self.model(dummy, verbose=False)
            print(f"🔥 {self.__class__.__name__} warmup完成，耗时: {(time.time()-t0)*1000:.1f}ms")
    ...
    def analyze(self, image_list: List[np.ndarray], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # print(f"[耗时] ROI裁剪: {(time.time()-t0)*1000:.1f}ms")
            if not roi_frames:
                return {
                    "Res":"",
                    "status": "fail",
            ...
            # print(f"[耗时] 亮度选择: {(time.time()-t0)*1000:.1f}ms")
            ...
            # print(f"[耗时] 图像预处理(填充+灰度): {(time.time()-t0)*1000:.1f}ms")

            # 5. YOLO11分类推理
            t0 = time.time()
            results = self.model(new_image, verbose=False)
            # print(f"[耗时] YOLO推理: {(time.time()-t0)*1000:.1f}ms")
            top_result = results[0]
            # 6. 解析分类结果
            top_class = self.classesname[top_result.probs.top1]   
            # print(top_class)
            # print(f"[耗时] analyze总耗时: {(time.time()-t_total)*1000:.1f}ms")
            return {
                "error":"",
                "status": "success",
                "Res":top_class,
            ...
            print(e)
            # print(f"[耗时] analyze总耗时(异常): {(time.time()-t_total)*1000:.1f}ms")
            return {
                "status": "fail",
                "Res":"",
                "error":e,
...
class DtextAlgorithm(SCAlgorithmBase):
    ...
    def _load_model(self):
        ...
        try:
            ...
            print(f"✅ {self.__class__.__name__} 模型加载完成: {self.model_path}")
            # warmup：用灰度图进行一次前向，避免首次推理耗时过长
            t0 = time.time()
            dummy = np.zeros((128, 128, 3), dtype=np.uint8)
            self.detector.detect_single_image(dummy)
            print(f"🔥 {self.__class__.__name__} warmup完成，耗时: {(time.time()-t0)*1000:.1f}ms")
        except Exception as e:
            raise RuntimeError(f"❌ {self.__class__.__name__} 模型加载失败: {str(e)}")
...
class AlgorithmScheduler:
    ...
    def run(self, image_list,input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # print(f"[耗时] AlgorithmScheduler.analyze: {(time.time()-t0)*1000:.1f}ms")
        max_brightness_idx=result["max_brightness_idx"]
        if(result["status"]=="fail"):
            # print(f"[耗时] AlgorithmScheduler.run总耗时(失败): {(time.time()-t_run)*1000:.1f}ms")
            return {
                "Res": {
                    "jxt_yc":[],
                    "jxt_yx":[]
        ...
        if(yxyc_type=="yx"):
            # print(f"[耗时] AlgorithmScheduler.run总耗时: {(time.time()-t_run)*1000:.1f}ms")
            return {
                "Res": {
                    "jxt_yc":[],
                    "jxt_yx":[{"id":cime_ID,
