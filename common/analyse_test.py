...
# 不再依赖 CIME 和点表，仅保留图元文件查找功能
class SimpleFileFinder:
    """简单文件查找器，仅用于图元文件索引，不做 CIME/点表加载"""
    def __init__(self):
        self._path_map = {}

    def add_dirs(self, root_dirs):
        for root_dir in root_dirs:
            if not os.path.isdir(root_dir):
                continue
            for root, dirs, files in os.walk(root_dir):
                for f in files:
                    self._path_map[f] = os.path.join(root, f)

    def find_file(self, filename):
        return self._path_map.get(filename)
...
def jxp_NameString(input_instruction: str):
    """
    仅解析 G 文件 p_NameString 指令中的 col、ln、dx、sql 等参数，
    不再查询外部 CIME 或点表，点位信息直接从 G 文件元素属性获取。
    """
    # 解析指令
...
class AnalyseG2:
    ...
    彻底消灭递归内的重复初始化和IO开销
    """
    def __init__(self, g_path: str, file_finder, button_config: dict = None):
        self.g_path = g_path
        self.file_finder = file_finder  # 仅用于图元文件查找，不做点号匹配
    ...
    def get_normal_point_pname(self, label, w=40, h=20, mode="yx"):
        ...
        for dtext in dtexts:
            ...
            # 解析 p_NameString 仅用于获取 col/ln/dx 等布局参数，不做外部点号匹配
            jxpdict = jxp_NameString(p_NameString)
            # 不再使用 jxpdict["yx"] 覆盖 keyid
            ...
            if devref == "":
                ...
                if devref:
                    ...
                    typath = self.file_finder.find_file(tyname)
                    if typath:
                ...
                elif "SH_光字牌sh.gzp.icn.g" in devref:
                    dtext_x, dtext_y, dtext_w, dtext_h = jisuan(dtext_x, dtext_y+1, 40, 18, rotate_num, scale_w, scale_h)
                else:
    ...
    def get_DollyBreaker(self):
        ...
        for dtext in dtexts:
            ...
            rect2 = (int(dtext_x2)-2, int(dtext_y2)-2, int(dtext_x2+dtext_w2), int(dtext_y2+dtext_h2))
            ...
            if gid and name1 and name2:
                tree["cimeid"] = [keyid1 if keyid1 else rtkeyid1, keyid2 if keyid2 else rtkeyid2]
                tree["cimename"] = [name1, name2]
    ...
    def getSpecialGZP(self):
        """
        光字牌分页处理。
        不再依赖外部 CIME/点表进行点号匹配，因此不再生成分页点位，
        Gzp 元素本身已在 get_normal_point_pname 中作为普通遥信点提取。
        """
        if not self.root:
            return []
    ...
    def getallpoint(self):
        ...
        if not self.root:
            ...
            return
        # 遥信类元素
        yx_labels = ["GroundDisconnector", "Disconnector", "CBreaker", "Gzp"]
    ...
    def get_gzp_navigation_buttons(self):
        ...
        for point in self.points:
            ...
            if not child_name:
                continue
            child_path = self.file_finder.find_file(child_name)
            if not child_path:
                continue
...
# ===================== 7. 递归构建节点树（零重复初始化） =====================
def FromGstoMap(file_finder, jgdir: str, g_path: str, button_config: dict = None):
    """有光字牌分页情况"""
    all_nodes = []
    processed_names = set()
    ...
    def recursive_build_nodes(current_g_path, nodename, parent_node_name=None):
        ...
        try:
            ...
            rootg = AnalyseG2(current_g_path, file_finder, button_config)
            rootg.getallpoint()
                ...
                for ssn_node in ssnodes:
                    for child_name in ssn_node.get("children_names", []):
                        if child_name in processed_names:
                            continue
                        child_path = file_finder.find_file(child_name)
                        if child_path:
...
# ===================== 7. 递归构建节点树（零重复初始化） =====================
def FromGstoMapSGZ(file_finder, jgdir: str, g_path: str, button_config: dict = None):
    """无光字牌分页情况"""
    all_nodes = []
    processed_names = set()
    nodename = os.path.basename(g_path)
...
if __name__ == "__main__":
    # 配置参数
    test_g = "/workspace/test/scada/g"
    test_main_g = "/workspace/test/scada/g/SH.新金阳站总图.fac.pic.g"

    tyyuandir="/workspace/test/tuyuan"
    # 1. 一次性构建图元文件路径索引，不做 CIME/点表解析
    file_finder = SimpleFileFinder()
    file_finder.add_dirs([test_g, tyyuandir])
    # 2. 递归构建所有节点
    button_config={}
    all_nodes = FromGstoMapSGZ(file_finder, test_g, test_main_g, button_config)
    print(all_nodes)
    rootg = AnalyseG2(test_main_g, file_finder, button_config)
    ssnodes = all_nodes[0]
    rootg.export_node_to_labelme(ssnodes,r"/workspace/test/scada/img/SH.新金阳站总图.fac.pic.jpg",save_path="/workspace/test/scada/result/SH.新金阳站总图.fac.pic.json")
